...
void main() {
    ...
    if (l <= 1.02) {
        ...
        vec3 r = vec3(2.0 * u.x, 1.0 - l, 2.0 * u.y) / (1.0 + l);

        vec3 extinction;
        result.rgb = sky_radiance(earthPos, r, normalize(sun_dir), extinction) + (fractal_fun(r, time) + 0.25);
    } else {
        const float avgFresnel = 0.17;
        result.rgb = skyIrradiance(earthPos.y, normalize(sun_dir).y) / M_PI * avgFresnel;
    }
	frag_color.a = 1.0;
