...
vec3 fractal_fun(vec3 v1, float t)
{
		vec3 fv1 = 1.0 * v1;
		vec3 fv2 = (fv1 * iResolution.x) / iResolution.y;
		...
		seed = floor(seed * iResolution.y);
		vec3 rnd = nrand3( seed );
		vec4 starcolor = vec4(pow(rnd.y,40.0));
	
		//Second Layer
		vec2 seed2 = (p2.xy + p2.z) * 2.0;
		seed2 = floor(seed2 * iResolution.y);
		vec3 rnd2 = nrand3( seed2 );
		starcolor += vec4(pow(rnd2.y,40.0));
		fractal_final += starcolor;
		return fractal_final.rgb;
}
...
void main()
{
	vec4 noise_sample = texture(noise_texture, vec3(uv, 85.0 / 127.0));
	vec4 geometry_color = texture(geometry_texture, uv);
	if (geometry_color.a > 0.0)
	{
		frag_color = vec4(geometry_color.rgb, 1.0);
	} else
	{
		vec3 v = normalize(view_dir);
		//fractals
		vec3 fractal = fractal_fun(v, time);
		//scattering
		...
		sky_color = HDR(sky_color + ((fractal + 0.25)));
		frag_color = vec4(sky_color.rgb, 1.0);
//		frag_color = noise_sample;
	}
	//frag_color = texture(reflectance_texture, uv);
}
